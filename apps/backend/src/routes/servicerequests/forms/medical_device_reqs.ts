...
router.post(
    ...
    authenticate(),
    authorize("employee"),
    sanitize(API.REQUESTS.CREATE.MEDICALDEVICE.REQ),
    async (req: SanitizedRequest<typeof API.REQUESTS.CREATE.MEDICALDEVICE.REQ>, res: Response) => {
            ...
            try {
                ...
                await PrismaClient.deviceRequest.create({
                    data: {
                        ...
                        device_needed: req.body.deviceNeeded,
                    },
                });
                res.status(200).send("OK" satisfies z.infer<typeof API.REQUESTS.CREATE.MEDICALDEVICE.RES>);
            } catch (error) {
...
router.post(
    ...
    authenticate(),
    authorize("employee"),
    sanitize(API.REQUESTS.UPDATE.MEDICALDEVICE.REQ),
    async (req: SanitizedRequest<typeof API.REQUESTS.UPDATE.MEDICALDEVICE.REQ>, res: Response) => {
        const requesterInfo = await PrismaClient.employee.findUnique({
            where: {
                id: req.body.requesterId,
            },
        ...
        if (requesterInfo === null) {
            ...
            try {
                ...
                await PrismaClient.deviceRequest.deleteMany({
                    where: {
                        serviceRequest: {
                            id: request.id,
                        },
