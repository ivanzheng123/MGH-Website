...
router.post(
    ...
    authenticate(),
    authorize("employee"),
    sanitize(API.REQUESTS.CREATE.LANGUAGE.REQ),
    async (req: SanitizedRequest<typeof API.REQUESTS.CREATE.LANGUAGE.REQ>, res: Response) => {
        ...
        if (requesterInfo === null) {
            ...
            try {
                ...
                await PrismaClient.languageRequest.create({
                    data: {
                        serviceRequest: {
...
router.post(
    ...
    authenticate(),
    authorize("employee"),
    sanitize(API.REQUESTS.UPDATE.LANGUAGE.REQ),
    async (req: SanitizedRequest<typeof API.REQUESTS.UPDATE.LANGUAGE.REQ>, res: Response) => {
        const requesterInfo = await PrismaClient.employee.findUnique({
            where: {
                id: req.body.requesterId,
            },
        ...
        if (requesterInfo === null) {
            ...
            try {
                ...
                await PrismaClient.languageRequest.deleteMany({
                    where: {
                        serviceRequest: {
                            id: request.id,
                        },
