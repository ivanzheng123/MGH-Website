...
//logic for completeing service requests
export const CompleteButton: FC<{ serviceRequestId: number }> = ({ serviceRequestId }) => {
  ...
  //if it is authenticated then allow for a button to show which allows for completion of task
  return (
    <button
      onClick={handleClick}
...
const columns: DataTableColumnDef<FrontendAPI["REQUESTS"]["VIEW"]["CREATED"]["RES"]> = [
  ...
  //type of request column
  {
    ...
    accessorKey: "type",
    //adds a space between the request type
    cell: ({ getValue }) => {
  ...
  //allows for edits in each service request
  {
    id: "edit",
    cell: ({ row, table }) => {
...
export const loader: RouteLoader = async ({ request, params }, context) => {
  ...
  if (isAuthenticated) {
    ...
    return await fetch(
      API.REQUESTS.VIEW.CREATED.ROUTE,
      await addTokenHeaderWithBody(await getAccessTokenSilently(), {
        id: user.id,
      })
    )
