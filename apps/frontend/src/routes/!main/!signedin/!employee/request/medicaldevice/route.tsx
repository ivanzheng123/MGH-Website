...
const formSchema = z.object({
  ...
  // SPECIFIC
  deviceNeeded: z.string().min(1, "Required").default(""),
});

type SubmitAction =
  | FrontendAPI["REQUESTS"]["CREATE"]["MEDICALDEVICE"]["REQ"]
  | FrontendAPI["REQUESTS"]["UPDATE"]["MEDICALDEVICE"]["REQ"];
...
export const Route: FC = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      if (useProvidedData) {
        ...
        if (providedData.deviceRequest !== null) {
          form.setValue("deviceNeeded", providedData.deviceRequest.device_needed);
        }
      }
    }, 100);
  }, []);
  ...
  useEffect(() => {
    ...
    const handleSetDevice = (e: Event) => {
      const custom = e as CustomEvent<string>;
      form.setValue("deviceNeeded", custom.detail);
    };
    ...
    window.addEventListener("set_device", handleSetDevice);
    ...
    return () => {
      ...
      window.removeEventListener("set_device", handleSetDevice);
    };
  }, [form]);
  ...
  return (
    <div className="p-4 flex justify-center">
      <div className={"panel w-fit p-4"}>
        <h1 className={"panel-title !p-0"}>Medical Device Form</h1>
        <hr className={"border-hospital-blue my-2"} />
        <p>Fill out this form to request a medical device.</p>
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit((data) => {
              ...
              if (useProvidedData) {
                submitter({
                  ...
                  // SPECIFIC
                  deviceNeeded: data.deviceNeeded,
                });
              } else {
                submitter({
                  requesterId: reqId,
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className={"gap-0"}>
                  <FormLabel className={"pb-2 cursor-pointer gap-1 text-md"}>
                    <div className={"text-red-500 font-normal"}>*</div>
                    <span className={"text-foreground"}>Medical Device Needed</span>
                    <FormMessage className={"ml-auto font-normal"} />
                  </FormLabel>
                  <FormControl>
                    <Input
...
export const action: RouteAction<SubmitAction> = async (data, args, context) => {
  ...
  if (isAuthenticated) {
    if ("requestId" in data && data.requestId !== undefined) {
      await fetch(
        API.REQUESTS.UPDATE.MEDICALDEVICE.ROUTE,
      ...
      await fetch(
        API.REQUESTS.CREATE.MEDICALDEVICE.ROUTE,
        await addTokenHeaderWithBody(await getAccessTokenSilently(), data)
      )
        .then((res) => {
          if (res.ok) {
