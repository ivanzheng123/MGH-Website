...
const formSchema = z.object({
  ...
  // SPECIFIC
  typeOfRequest: z.preprocess(
    (val) => val ?? "",
    z.enum(["Plumbing", "Electrical", "Cleaning", "General"], {
      message: "Required",
    })
  ),
  facility: z.string().min(1, "Required").default(""),
});

type SubmitAction =
  | FrontendAPI["REQUESTS"]["CREATE"]["MAINTENANCE"]["REQ"]
  | FrontendAPI["REQUESTS"]["UPDATE"]["MAINTENANCE"]["REQ"];
...
export const Route: FC = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      if (useProvidedData) {
        ...
        if (providedData.maintenanceRequest !== null) {
          form.setValue(
            "typeOfRequest",
            providedData.maintenanceRequest.typeOfRequest as "Plumbing" | "Electrical" | "Cleaning" | "General"
          );
          form.setValue("facility", providedData.maintenanceRequest.facility);
        }
      }
    }, 100);
  }, []);
  ...
  useEffect(() => {
    ...
    const handleMaintenanceType = (e: Event) => {
      const custom = e as CustomEvent<string>;
      const maint_type = ["plumbing", "electrical", "cleaning", "general"];
      const val = custom.detail;
      if (maint_type.includes(val)) {
        form.setValue(
          "typeOfRequest",
          (val.charAt(0).toUpperCase() + val.slice(1)) as "Plumbing" | "Electrical" | "Cleaning" | "General"
        );
      }
    };

    const handleSetFacility = (e: Event) => {
      const custom = e as CustomEvent<string>;
      form.setValue("facility", custom.detail);
    };
    ...
    window.addEventListener("set_maintType", handleMaintenanceType);
    window.addEventListener("set_facility", handleSetFacility);
    ...
    return () => {
      ...
      window.removeEventListener("set_maintType", handleMaintenanceType);
      window.removeEventListener("set_facility", handleSetFacility);
    };
  }, [form]);
  ...
  return (
    <div className="p-4 flex justify-center">
      <div className={"panel w-fit p-4 "}>
        <h1 className={"panel-title !p-0"}>Maintenance Form</h1>
        <hr className={"border-hospital-blue my-2"} />
        <p>Fill out this form to file a maintenance request.</p>
        <Form {...form}>
          <form
            ref={formRef}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className={"gap-0"}>
                  <FormLabel className={"pb-2 cursor-pointer gap-1 text-md"}>
                    <div className={"text-red-500 font-normal"}>*</div>
                    <span className={"text-foreground"}>Maintenance Type</span>
                    <FormMessage className={"ml-auto font-normal"} />
                  </FormLabel>
                  <Select {...field} onValueChange={field.onChange}>
                    <FormControl>
                    ...
                    <SelectContent className={"panel !p-0 !rounded-xl"}>
                      {formSchema.shape.typeOfRequest._def.schema.options.map((value) => (
                        <Fragment key={`${field.name}$${value}`}>
                          <SelectItem
                            value={value}
                            className={
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className={"gap-0"}>
                  <FormLabel className={"pb-2 cursor-pointer gap-1 text-md"}>
                    <div className={"text-red-500 font-normal"}>*</div>
                    <span className={"text-foreground"}>Maintenance Facility</span>
                    <FormMessage className={"ml-auto font-normal"} />
                  </FormLabel>
                  <FormControl>
                    <Input
