...
const formSchema = z.object({
  ...
  // SPECIFIC
  destination: z.string().min(1, "Required").default(""),
  vehicle: z.string().min(1, "Required").default(""),
});

type SubmitAction =
  | FrontendAPI["REQUESTS"]["CREATE"]["TRANSPORTATION"]["REQ"]
  | FrontendAPI["REQUESTS"]["UPDATE"]["TRANSPORTATION"]["REQ"];
...
export const Route: FC = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      if (useProvidedData) {
        ...
        if (providedData.transportationRequest !== null) {
          form.setValue("destination", providedData.transportationRequest.destination);
          form.setValue("vehicle", providedData.transportationRequest.vehicle);
        }
      }
    }, 100);
  }, []);
  ...
  useEffect(() => {
    ...
    const handleTransportationDestination = (e: Event) => {
      const custom = e as CustomEvent<string>;
      const val = custom.detail;
      form.setValue("destination", val);
    };

    const handleTransportationVehicle = (e: Event) => {
      const custom = e as CustomEvent<string>;
      const val = custom.detail;
      form.setValue("vehicle", val);
    };
    ...
    window.addEventListener("set_transportation_destination", handleTransportationDestination);
    window.addEventListener("set_transportation_vehicle", handleTransportationVehicle);
    ...
    return () => {
      ...
      window.removeEventListener("set_audioOrVisualNeeded", handleTransportationDestination);
    };
  }, [form]);
  ...
  return (
    <div className="p-4 flex justify-center">
      <div className={"panel w-fit p-4"}>
        <h1 className={"panel-title !p-0"}>Transportation Form</h1>
        <hr className={"border-hospital-blue my-2"} />
        <p>Fill out this form to request transportation for a patient.</p>
        <Form {...form}>
          <form
              ...
              render={({ field }) => (
                <FormItem className={"gap-0"}>
                  ...
                  <FormControl>
                    <Input
                      {...field}
                      placeholder={"Specify a mode of transport"}
                      className={
                        "panel-interactive w-full  placeholder:text-foreground/75 !text-foreground !rounded-xl !ring-0 !cursor-text"
                      }
                    />
